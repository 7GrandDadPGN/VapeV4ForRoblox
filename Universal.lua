...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/WhichChapter/HydraWare/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				GuiLibrary.SelfDestruct = function() warningNotification('HydraWare', 'No escaping the private members :)', 10) end
