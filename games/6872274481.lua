...
local Fly local LongJump run(function() local Value local VerticalValue local WallCheck local PopBalloons local TP local ProgressBarToggle local rayCheck = RaycastParams.new() rayCheck.RespectCanCollide = true local up, down, old = 0, 0
...
Fly = vape.Categories.Blatant:CreateModule({
	...
	Function = function(callback)
		...
		if callback then
			...
			if ProgressBarToggle.Enabled then
				local frame = Instance.new("Frame")
				frame.AnchorPoint = Vector2.new(0.5, 0)
				frame.Position = UDim2.new(0.5, 0, 1, -200)
				frame.Size = UDim2.new(0.2, 0, 0, 20)
				frame.BackgroundTransparency = 0.5
				frame.BorderSizePixel = 0
				frame.BackgroundColor3 = Color3.new(0, 0, 0)
				frame.Visible = Fly.Enabled
				frame.Parent = GuiLibrary.MainGui

				local inner = frame:Clone()
...
ProgressBarToggle = Fly:CreateToggle({
	Name = 'Progress Bar',
	Default = true
})
